...
class Connections
{
    static public function showConnections($telegram_id, $telegram, $user, $update)
    {
        ...
        if($data['status'] === 'error' || empty($data)) {
            $text = __("Error! Try again later.", $user['language']);

        } else {
            if(empty($data['connections'])) {
                $text = __('You do not have connections', $user['language']);
            } else {
                $i=1;
                $text='';
                foreach ($data['connections'] as $item) {
                    ...
                    $text.=$i.'. '.$user_name_text.' '.__('created on', $user['language']).' '.date('j/m/y',strtotime($item['created_on']))."\n";
                    $i++;
                }
            }
